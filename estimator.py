...
class AnomalyDetector():
    def __init__(self, ours=True, input_shape=(3, 1024, 2048), seed=0, fishyscapes_wrapper=True, verbose=False):
    ...
    def estimator_image(self, image):
        ...
        # run synthesis
        syn_input = {'label': label_tensor.unsqueeze(0).cuda(), 'instance': instance_tensor.unsqueeze(0).cuda(),
                     'image': image_tensor.unsqueeze(0).cuda(), "mask": mask}
        ...
        diss_pred_img = Image.fromarray(diss_pred_img).resize((image_og_w, image_og_h))
        ...
        img_box = draw_total_box(np.array(image), result[0], debug=False)
        out = {"anomaly_map": diss_pred_img, "segmentation": seg_img,
               "synthesis": synthesis, "softmax_entropy": entropy, "perceptual_diff": perceptual_diff,
               "softmax_distance": distance, "box": img_box}
