...
image_path = 'sample_images/road_anomaly_example.png'
...
# inputs
onnx_model_path = '../road_obstacles/demo_files/onnx_models'
...
segmentation_session = onnxruntime.InferenceSession(segmentation_onnx, providers=['CUDAExecutionProvider'])
synthesis_session = onnxruntime.InferenceSession(synthesis_onnx, providers=['CUDAExecutionProvider'])
dissimilarity_session = onnxruntime.InferenceSession(dissimilarity_onnx, providers=['CUDAExecutionProvider'])
...
# display(synthesis_final_img.resize((2048, 1024)))
...
diss_pred = diss_pred[:, 1, :, :] * weights[0] + entropy_tensor.numpy() * weights[1] + perceptual_diff_tensor.numpy() * \
            weights[2] + distance_tensor.numpy() * weights[3]
diss_pred = (diss_pred * 255).astype(np.uint8).squeeze()
diss_pred[diss_pred < 128] = 0
diss_pred[diss_pred >= 128] = 255
