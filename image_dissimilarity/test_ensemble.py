...
def grid_search(model_num=4):
    ...
    for weights in product(w, repeat=model_num):
        ...
        if score_fp < best_score:
            ...
            print('>BEST SO FAR %s Score_FP: %.3f Score_ROC:%.3f Score_AP:%.3f' % (
            best_weights, best_score, best_roc, best_ap))
...
def evaluate_ensemble(weights_f):
    ...
    with torch.no_grad():
        for i, data_i in enumerate(test_loader):
            ...
            soft_pred = outputs[:, 1, :, :] * weights_f[0] + entropy * weights_f[1] + mae * weights_f[2] + distance * \
                        weights_f[3]
...
if __name__ == '__main__':
    ...
    print(
        'Best weights: %s Score_FP: %.3f Score_ROC:%.3f Score_AP:%.3f' % (best_weights, best_score, best_roc, best_ap))
