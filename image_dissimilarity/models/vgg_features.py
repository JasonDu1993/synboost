...
class VGGSPADE(torch.nn.Module):
    ...
    def forward(self, img, semantic_img):
        h_relu1 = self.slice3(
            self.norm_layer_2(self.slice2(self.norm_layer_1(self.slice1(img), semantic_img)), semantic_img))
        h_relu2 = self.slice6(
            self.norm_layer_4(self.slice5(self.norm_layer_3(self.slice4(h_relu1), semantic_img)), semantic_img))
        h_relu3 = self.slice10(self.norm_layer_7(self.slice9(
            self.norm_layer_6(self.slice8(self.norm_layer_5(self.slice7(h_relu2), semantic_img)), semantic_img)),
                                                 semantic_img))
        h_relu4 = self.slice14(self.norm_layer_10(self.slice13(
            self.norm_layer_9(self.slice12(self.norm_layer_8(self.slice11(h_relu3), semantic_img)), semantic_img)),
                                                  semantic_img))
...
class VGG19_difference(torch.nn.Module):
    ...
    def forward(self, X, Y):
        ...
        feat1 = torch.mean(torch.abs(x1 - y1), dim=1).unsqueeze(1)
        feat2 = torch.mean(torch.abs(x2 - y2), dim=1).unsqueeze(1)
        feat3 = torch.mean(torch.abs(x3 - y3), dim=1).unsqueeze(1)
        feat4 = torch.mean(torch.abs(x4 - y4), dim=1).unsqueeze(1)
        feat5 = torch.mean(torch.abs(x5 - y5), dim=1).unsqueeze(1)
        ...
        perceptual_diff = self.weights[0] * img_1 + self.weights[1] * img_2 + self.weights[2] * img_3 + self.weights[
            3] * img_4 + self.weights[
                              4] * img_5
