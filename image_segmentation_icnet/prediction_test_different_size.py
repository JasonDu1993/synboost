...
def WholeTest(args, model, size=1.0):
    ...
    print('model loaded')
    img_list = [os.path.join(args.input_dir, image) for image in os.listdir(args.input_dir) if
                '.jpg' or '.png' in image]
    for i in img_list:
        if os.path.isdir(i):
            continue
        name = i
        image_save_path = os.path.join(save_dir, 'temp', 'leftImg8bit', 'val',
                                       os.path.basename(name)[:-4] + '_leftImg8bit' + os.path.basename(name)[-4:])
        ...
        with torch.no_grad():
            ...
            cv2.imwrite(
                os.path.join(save_dir, 'temp', 'gtFine', 'val', os.path.basename(name)[:-4] + '_instanceIds.png'),
                dst)
...
if __name__ == '__main__':
    """cmd:--input_dir /zhoudu/workspaces/synboost/sample_images --output_dir /zhoudu/workspaces/road_obstacles/debug --resume /zhoudu/workspaces/road_obstacles/models/image-segmentation/icnet_final.pth --arch icnet
    """
    parser = argparse.ArgumentParser(description='PyTorch \
                Segmentation Crop Prediction')
    parser.add_argument('--input_dir', type=str, default="/zhoudu/workspaces/synboost/sample_images",
                        help='test dataset folder')
    parser.add_argument("--input_disp_dir", type=str, default=None)
    parser.add_argument('--output_dir', type=str, default="/zhoudu/workspaces/road_obstacles/debug",
                        help='output directory of the model_test, for saving the seg_models')
    parser.add_argument("--resume", type=str,
                        default="/zhoudu/workspaces/road_obstacles/models/image-segmentation/icnet_final.pth")
    ...
    test_list = [os.path.join(args.input_dir, image) for image in os.listdir(args.input_dir) if
                 '.jpg' or '.png' in image]
